...
function DashBoard() {
  ...
  const can = (perm) => !isStudent && (isSuperAdmin || userPermissions.includes(perm));
  ...
  const renderSection = () => {
    ...
    switch (section) {
      ...
      case "Register_Courses":
        if ((isStudent || isSuperAdmin) && (studentCan("view_courses") || isSuperAdmin)) {
      ...
      case "advisor_chat":
        return ["professor", "admin", "super_admin"].includes(roleName)
          ? <AdvisorChat />
          : <Denied />;
      ...
      return (
        <div className="header-page-actions">
          ...
          <button
            ...
            onClick={() =>
              document.getElementById("ep-save-trigger")?.click()
            }
          >
            Save Changes
          </button>
